...
allHrsArray.forEach(ele => ele.style.backgroundColor = "antiquewhite");
...
let box17 = document.querySelector(`div[class = "box seventeen"]`);
box17.style.display = 'none';

// Change the border of all the hr element from solid to dashed type
allHrsArray.forEach(ele => ele.style.border = "1px dashed tomato");
...
while(document.querySelector(`div[class = "box one"]`).children.length > 0) {
  document.querySelector(`div[class = "box one"]`).children[0].remove();
}


// Replace all the elements inside box 1 with the para (you created above)
document.querySelector(`div[class = "box one"]`).append(para);
...
*/
let box16 = document.querySelector(`div[class = "box sixteen"]`);
box16.parentNode;
box16.childNodes;
...
para = "Append inserts as last child" ;
document.querySelector(`div.two`).append(para);

// Select box 3 and prepend a new paragraph element with content "Prepend inserts as first child" just before hr element.
para = "Prepend inserts as first child" ;
document.querySelector(`div.three`).prepend(para);

// Change the border of box 4 to '1px solid black'
document.querySelector(`div.four`).style.border = "1px solid black" ;

// Change the border radius of box 5 to 10px.
document.querySelector(`div.five`).style.borderRadius = "10px" ;

// Change the text color of box 6 to black.
document.querySelector(`div.six`).style.color = "black";

// Change the font size of the para inside box 1 to 0.8rem.
document.querySelector(`div.one`).style.fontSize = ".8rem";

// Change the background of all the alternate boxes (1, 3, 5, ....) to aliceblue
let all = Array.from(document.querySelectorAll(`div.box`));
let odd = all.filter(ele => all.indexOf(ele) % 2 === 0);
odd.forEach(ele => ele.style.backgroundColor = "aliceblue");

// add a class named "awesome-box" to the box 6 using classList property of DOM element.
document.querySelector(`div.six`).classList.add("awesome-box");

// Using the toggle classList property toggle the class `awesome-box` from box 2
document.querySelector(`div.two`).classList.toggle("awesome-box");

// Using the remove classList proeprty remove the class `awesome-box` from box 4
document.querySelector(`div.four`).classList.remove("awesome-box");
...
document.querySelector(`div.nine`).append(btn);
...
imgElm.setAttribute(`src`, "https://unsplash.com/photos/VF_t2tqx0rg") ;

// Select the box 7 using class seven
document.querySelector(`div.seven`);

// Remove all the elements form box seven
while(document.querySelector(`div.seven`).children.length > 0) {
  document.querySelector(`div.seven`).children[0].remove();
}

// Append the imgElm to the box no 7
document.querySelector(`div.seven`).append(imgElm);
